...
- (void)pushAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    WPTransitionInteractiveVC * toVC = (WPTransitionInteractiveVC *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    ...
    if ([[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey] isKindOfClass:[UINavigationController class]]) {
        UINavigationController * na = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
...
//    
    //获取点击的cell
    UIImageView * imageView  = fromVC.gestureImageView;
    ...
    UIView *imageViewCopy = [imageView snapshotViewAfterScreenUpdates:NO];
    imageViewCopy.frame = [imageView convertRect:imageView.bounds toView:containerView];
    //设置动画前的各个控件的状态
    fromVC.gestureImageView.hidden = YES;
...
- (void)popAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    //通过viewControllerForKey取出转场前后的两个控制器，这里toVC就是转场后的VC、fromVC就是转场前的VC
    ViewController * toVC;
    WPTransitionInteractiveVC * fromVC = (WPTransitionInteractiveVC *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    if ([[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey] isKindOfClass:[UINavigationController class]]) {
        UINavigationController * na = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
...
//    UIView * fromView = [transitionContext viewForKey:UITransitionContextFromViewKey];
//
//    UIImageView * imageView  = toVC.gestureImageView;
    UIView *containerView = [transitionContext containerView];
    ...
    [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{
        imageViewCopy.frame = [toVC.gestureImageView convertRect:toVC.gestureImageView.bounds toView:containerView];
        bgView.alpha = 0;
    } completion:^(BOOL finished) {
        //由于加入了手势必须判断
        [transitionContext completeTransition:![transitionContext transitionWasCancelled]];
        if ([transitionContext transitionWasCancelled]) {
            ...
            toVC.gestureImageView.hidden = NO;
        }
